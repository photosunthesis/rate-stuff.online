...
export function RatingCardSkeleton({
	...
	const header =
		hideAvatar || variant === "user" ? (
			...
			>
				<div
					className={`h-3 bg-neutral-800/40 rounded-full w-28 ${variant === "user" ? "" : "mb-2"}`}
				/>
			</div>
		) : (
			<div className="flex items-center gap-3">
				...
				<div className="flex-1 min-w-0">
					<div className="flex items-center gap-1 flex-wrap text-base">
						<div className="h-3 bg-neutral-800/40 rounded-full w-48 mb-2" />
					</div>
				</div>
			</div>
		);
	...
	const imagePlaceholder = showImage ? (
		<div className={`${indent} mb-3`}>
			<div className="aspect-video bg-neutral-900/40 rounded-2xl" />
		</div>
	) : null;
	...
	return (
		<div className="px-8 py-4 animate-skeleton-pulse pointer-events-none">
			...
			<div className={`${titleMargin} ${indent}`}>
				<div className="h-4 bg-neutral-800/40 rounded-full w-56" />
			</div>

			{imagePlaceholder}

			<div className={`${indent} mb-3`}>
				<div className="h-3 bg-neutral-800/40 rounded-full w-full mb-2" />
				<div className="h-3 bg-neutral-800/40 rounded-full w-5/6 mb-2" />
				<div className="h-3 bg-neutral-800/40 rounded-full w-3/4" />
			</div>

			<div className={`flex flex-wrap gap-2 mb-3 ${indent}`}>
				<div className="h-6 w-16 bg-neutral-800/40 rounded-full" />
				<div className="h-6 w-12 bg-neutral-800/40 rounded-full" />
			</div>

			<div className={`${noIndent ? "" : "ml-11"} flex items-center gap-3`}>
				<div className="h-6 w-24 bg-neutral-800/40 rounded-full" />
				<div className="h-6 w-16 bg-neutral-800/40 rounded-full" />
			</div>
		</div>
	);
